...
class SADH(Optimizer):
    """
    Implementation of SADH: Second-Order Step Size Methods Using the Smoothed Absolute Diagonal Hessian Estimates
    """
    def __init__(self, params, lr:float=6e-2, betas:tuple=(0.96, 0.99),rho:float=1e-2, weight_decay:float=0.0, eps:float=1e-12, update_period:int=10, diag_hessian_distribution:str='rademacher'):
        ...
        super(SADH, self).__init__(params, defaults)
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if 'hessian' in state and (step % self.update_period == 0 or hessian is not None):
                    hessian_term.mul_(beta2).add_(torch.abs(state['hessian']), alpha=1.0-beta2)
                denom = hessian_term + group['eps']
