...
class Sophia(Optimizer):
    """
    Unofficial implementation of Sophia with the Hutchinson diagonal Hessian estimator
    "Sophia: A Scalable Stochastic Second-order Optimizer for Language Model Pre-training"
    https://arxiv.org/abs/2305.14342

    """
    def __init__(self, params, lr:float=6e-2, betas:tuple=(0.96, 0.99),rho:float=1e-2, weight_decay:float=0.0,eps:float=1e-12, update_period:int=10, diag_hessian_distribution:str='gaussian'):
        if not 0.0 <= lr:
    ...
    def diagonal_hessian_estimate(self):
        """
        Compute the Hutchinson diagonal Hessian estimates 
        """
    ...
    def step(self, closure=None, hessian=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                without_clip = momentum /torch.clip(hessian_term, min=group['eps'])
                update = (momentum/torch.clip(hessian_term, min=group['eps'])).clamp_(min=-group['rho'], max=group['rho'])
